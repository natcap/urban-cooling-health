...
df_base <- df_i %>%
  filter(year == baseline_year, lc_scenario == "scenario0") %>%
  select(GSS_CODE, 
...
df_change_00 <- df_i %>%
  ...
  select(-baseline) %>%
  # filter(climate_lc != paste0(baseline_year, "_scenario0")) %>%
  as.data.frame() 
...
f <- file.path(dir.fig, paste("temp_scenarios_fromshp", "line_dif", ind, 'both', ymd, "01.png", sep = '_'))
...
p
f <- file.path(dir.fig, paste("temp_scenarios_fromshp", "line_dif", ind, 'both', ymd, "02.png", sep = '_'))
func_ggsave(f, w = 7/2, h = 5, dpi = 300, save_png = T)
```
