...
p <- stats %>%
  ...
  ggplot(data = ., 
         aes(x = cause, y = mean, 
             group = lc_scenario, 
             # group = interaction(cause, lc_scenario), 
             # fill = lc_scenario, 
             color = lc_scenario)) +
...
#-----------------------------
dir.create(dir_fig, showWarnings = FALSE, recursive = TRUE)
f <- file.path(dir_fig, paste0("health_monte_carlo_violins_by_cause_Scenario_dodged_", year_range, '_', ymd, "-2.png")); f
ggsave(filename = f, plot = p, width = 7/2, height = 5, dpi = 300)
