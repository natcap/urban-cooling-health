...
long2 <- 
  map_dfr(fs, ~ read_csv(.x, show_col_types = FALSE) |>
  # mutate(Scenario = str_extract(.x, "s\\d+(?:_s\\d+)*")) |> ## only scenario strings
  mutate(Scenario = str_extract(.x, "s\\d+_s\\d+(?:_\\d+_\\d+)?")) |> ## scenario + years
...
p <- 
  ggplot(long2, aes(x = cause, y = value, fill = Scenario)) +
  ...
  stat_summary(
    ...
    fun = mean, geom = "text", position = pd, show.legend = F, 
    vjust = -0.6,
    # hjust = -0.6,
    size = 3
  ) +
  ...
  coord_flip() +
  theme(
    legend.position = c(legend.position.x, .75),
...
ggsave(filename = f, plot = p, width = 7, height = 5, dpi = 300)
