...
for (yr in yr_list) {
  ...
  dat_ <- dat %>%
    pivot_longer(cols = matches("00|10|60"),
                 names_to = "ind",
                 values_to = "number")
  ...
  # Save as PNG
  tmap_save(tm = map, filename = paste0("./figures/health_", icd_postfix, '_', yr, ".png"), 
            width = 16, height = fig.height, units = "in", dpi = 300)
...
dat <- dat_bor %>%
  filter(date == yr) %>%
  left_join(gla_bor, ., by = c('GSS_CODE' = 'geogcode')) %>%
  pivot_longer(cols = 13:18, names_to = 'ind', values_to = 'death_case') %>%
  mutate(
    # 1 hectare = 10,000 m²
...
str(dat_w)
names(dat_w)
message("Columns: \n", paste(names(dat_w)[14:19], collapse = "\n"))
