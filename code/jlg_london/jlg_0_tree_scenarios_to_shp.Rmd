...
dir.raw   <- 'D:/natcap/urban-cooling-health/data/jlg_tree_planting/'
dir.share <- 'G:/My Drive/NatCap/projects/KCL_Welcome/jlg-london/tree_planting/'
```
```{r}
...
buffered_sf <- st_buffer(points_sf, dist = diameter/2)
...
func_plot_map <- function(buffered_sf, title_pre = 'data2') {
  ...
  ggplot() +
    ...
    labs(title = paste0(title_pre, " - tree planting in Camden (n = ", n_tree, ")"),
         subtitle = paste0(diameter, "m Diameter (EPSG:27700)"))
  
  f <- paste0(dir.share, 'figures/', title_pre, '_map.png')
