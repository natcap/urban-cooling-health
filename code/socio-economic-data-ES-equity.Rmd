...
ggplot() +
  ...
  geom_rug(data = raw_data_long, aes(x = x_raw), 
           sides = "b", alpha = 0.1, length = unit(0.03, "npc"), color = "gray20") +
  
  # --- Layer 2: Confidence Interval ---
  geom_ribbon(data = curve_data, aes(x = x, ymin = y - 1.96 * se, ymax = y + 1.96 * se), 
              fill = "#2c7bb6", alpha = 0.2) +
...
ggplot() +
  ...
  geom_density(data = raw_data_long, 
               aes(x = x_raw, 
  ...
  labs(title = gam_plot_title,
       caption = "Curves represent the partial effect of each metric (centered at mean); Gray area = Distribution of observed data density",
       x = "Vulnerability Metric (Standardized Z-Score)",
       y = "Predicted Change in Cases (Relative to Mean)") +
...
f <- paste0('./figures/', 'health_outcome_equity_esGAM_', lc_snr, '_', ymd, '_actual3.png')
ggsave(plot = last_plot(), filename = f, width = 7, height = 5, units = 'in', dpi = 300)
```
...
for (m in metrics) {
  ...
  data_subset <- raw_data_long %>% filter(Metric == m)
  curve_subset <- curve_data %>% filter(Metric == m)
