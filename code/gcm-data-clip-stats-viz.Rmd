...
p <- 
  df_ch_wc %>%
  filter(year >= 2020) %>%
  ggplot(., aes(x = factor(year),  # factor(year) on x-axis ensures discrete spacing
                y = .data[[value_col]], 
  ...
  geom_boxplot(
    ...
    # position_dodge2() must be consistent across all layers (boxplot and both stat_summary calls)
    position = position_dodge2(width = 0.75, padding = 0.1, preserve = "single"),
    show.legend = FALSE,
    outlier.shape = NA, width = 0.6, linewidth = 0.2, 
    # fill = "grey85", 
    # color = "grey40", 
    alpha = 0.4) +
  ...
  stat_summary(
    ...
    geom = "text", 
    # color = "#f03b20", 
    aes(label = after_stat(sprintf("%.1f", y))), show.legend = F,
    vjust = -1.2,
    hjust = 0.8,
    size = 3
  ) +
...
# --- 3) save ---
f <- paste0(dir.fig, "/T_change_borough_overTime_", n_scenario, "s.png")
...
# --- 3) save ---
f <- paste0(dir.fig, '/T_change_borough_by_model-quantile_ggplot_avg_years-v-', ymd, '.png')
