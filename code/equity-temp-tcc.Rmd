...
df_pd <- readRDS('./data/ucm_output_shp_stats_productivity_by_lsoa.rds') %>%
...
df_cont <- df_svi %>%
  ...
  # rename(
  #   `Age over 75`     = p75over,
  #   `Ethnic minority` = pBAME,
  #   `Low income`      = pIncomeDeprivation,
  #   `Age below 5`     = p5under
  # ) %>%
  as.data.frame()
...
for (m in metrics_renamed) {
  
  # 1. Fit the Non-Linear GAM (The Winner)
  # formula <- as.formula(paste("y ~ s(", m, ")"))
  formula_gam <- as.formula(bquote(y ~ s(.(as.name(m)))))
  gam_model <- gam(formula_gam, data = df_cont_scaled, method = "REML")
  ...
  formula_lm <- as.formula(bquote(y ~ .(as.name(m))))
...
arrow_y <- y_floor + density_height  # Adjust this value to move arrows up/down
...
ggplot() +
  ...
  annotate("segment", 
           x = 0, xend = -2,       # Start near 0, point to -2
           y = arrow_y, yend = arrow_y, 
           colour = "gray40", alpha = 0.6, 
           arrow = arrow(length = unit(arrow_len, "cm"), type = "open")) +
  
  annotate("text", 
           x = -1.5, y = arrow_y + 0.3*density_height, # Position text above arrow
  ...
  annotate("text", 
           x = 1.5, y = arrow_y + 0.3*density_height, 
           label = "More Vulnerable", 
           size = 2.2, color = "gray40", fontface = "italic") + 
  
  labs(
    # title = gam_plot_title,
    ...
    y = paste0("Predicted Change (Relative to Mean) in ",  ylab)) +
