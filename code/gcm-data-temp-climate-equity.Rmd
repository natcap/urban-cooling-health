...
```{r select the y variable}

yi <- 'temp_change'  # change in temperature: Year 2050 - Year 2021
yi <- 'temp_future'  # temperature in 2050
...
df_cat <- temp_svi %>%
  select(geoid, all_of(yi), ends_with("_q"), scenario, year) %>%   # Select 'cases' and all columns ending in '_q'
  # rename('yi' = yi) %>%
  pivot_longer(
...
func_plot_cld(data = df_cat2, data_letter = cld_labels) +
  labs(
    ...
    y = str_to_title(gsub('_future', '', yi)))
f <- paste0('./figures/', 'temp_equity_q_cld_', yi, '_', ymd, '.png')
...
func_plot_cld(data = df_cat2_svi, data_letter = cld_labels_svi) +
  ...
  labs(x = 'SVI quintile', y = 'Temperature')
f <- paste0('./figures/', 'temp_equity_q_cld_', yi, '_SVI_', ymd, '.png')
...
f <- paste0('./figures/', 'temp_equity_esGAM_', yi, '_', ymd, '_scaled.png')
ggsave(plot = last_plot(), filename = f, width = 7, height = 5, units = 'in', dpi = 300)
```
