...
map <- 
  ...
  tm_polygons("area_pct", 
              ...
              fill.free = T) +
  tm_facets(by = "type_name", ncol = 3) +
  tm_layout(legend.show = TRUE,
            legend.outside = T, 
            component.autoscale = F, 
            # shrink legend
            legend.text.size  = .4,   # shrink legend labels
            legend.title.size = .5,   # shrink legend title
            ...
            # inner.margins = c(bottom, left, top, right)
            inner.margins = c(0.01, 0.01, 0.05, 0.2),  # extra space on right
...
# Save as PNG
f <- paste0(here(), "/figures/building_stat_", ymd, "_pct.png"); f
tmap_save(tm = map, filename = f, width = 7, height = 3.6, units = "in", dpi = 300)
```
