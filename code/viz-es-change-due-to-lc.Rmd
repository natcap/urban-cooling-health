...
## Production map (revised six-scenario analysis)

This section replaces the three manually saved `viz_sf_*` objects with the
reviewed borough summary and population-weighted health rasters. It maps all
six scenarios using a common colour scale within each outcome row, so Green
and Target strategies can be compared at the same 10%, 20% and 30% canopy
budgets. Set `URBAN_COOLING_DATA_ROOT` before knitting.
...
available and are not implied by the colour variation.
