...
# install.packages(c("sf","dplyr","ggplot2","viridisLite","rnaturalearth"))
library(sf)
library(dplyr)
library(ggplot2)
...
dat_stat_mm <- dat_stat %>%
  ...
  left_join(., sta %>% select(src_id, station_latitude, station_longitude), by = 'src_id') %>%
  as.data.frame()
